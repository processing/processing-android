...
/**
 * Build Gradle plugin needed to use aar files as dependencies in a pure java library project.
 * Adapted from the following plugin by nekocode
 * https://github.com/nekocode/Gradle-Import-Aar
 * Ported to kotlin, and made specific to the needs of the Android mode build process (i.e.: this plugin
...
open class ImportAar : Plugin<Project> {
    ...
    class AarJarArtifactTransform : ArtifactTransform() {
        ...
        override fun transform(file: File): MutableList<File> {
            ...
            // val names = file.getPath().split(Pattern.quote(File.separator))
            // print ("NAMES ")
            // println (names)
            // print ("NAME ")
            // println (file.getName())
            // val aarName = names[names.length - 4].replace(".aar", "")
