 ...
 * @see StringList
 */
open class IntList : Iterable<Int?> {
    ...
    constructor(iter: Iterable<Any?>) : this(10) {
        for (o in iter) {
            if (o == null) {
                ...
                append(PApplet.parseInt(o.toString().trim { it <= ' ' }))
            }
        }
        crop()
    }
...
//    data = outgoing;
    ...
    // same as splice
    fun insert(index: Int, values: IntArray) {
        ...
        val temp = IntArray(count + values.size)
...
//    }
    ...
    fun insert(index: Int, list: IntList) {
...
//  public Iterator<Integer> valueIterator() {
    ...
    fun getSubset(start: Int, num: Int): IntList {
        val subset = IntArray(num)
        System.arraycopy(data, start, subset, 0, num)
        return IntList(subset)
    }
    ...
    companion object {
        @JvmStatic
        fun fromRange(stop: Int): IntList {
            return fromRange(0, stop)
        }

        @JvmStatic
        fun fromRange(start: Int, stop: Int): IntList {
            val count = stop - start
            val newbie = IntList(count)
            for (i in 0 until count) {
                newbie[i] = start + i
            }
            return newbie
