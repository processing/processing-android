 ...
 * @see StringDict
 */
open class FloatDict {
    /** Number of elements in the table  */
    protected var count = 0
    protected var keys: Array<String?>
    protected var values: FloatArray
     ...
     * @nowebref
     */
    constructor(keys: Array<String?>, values: FloatArray) {
    ...
    constructor(pairs: Array<Array<Any>>) {
        ...
        values = FloatArray(count)
        for (i in 0 until count) {
            keys[i] = pairs[i][0] as String
            values[i] = pairs[i][1] as Float
            indices[keys[i]!!] = i
        }
    }
    ...
    fun resize(length: Int) {
        ...
        val newValues = FloatArray(length)
    ...
    // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
    inner class Entry internal constructor(var key: String?, var value: Float)
    ...
    fun valueIterator(): MutableIterator<Float?> {
     ...
     */
    fun valueArray(array: FloatArray?): FloatArray {
        var array = array
    ...
    operator fun get(key: String?, alternate: Float): Float {
        val index = index(key)
        return if (index == -1) {
            alternate
        } else values[index]
     ...
     */
    operator fun set(key: String, amount: Float) {
        val index = index(key)
        if (index == -1) {
            create(key, amount)
        } else {
    ...
    fun setIndex(index: Int, key: String, value: Float) {
     ...
     */
    fun add(key: String, amount: Float) {
        val index = index(key)
        if (index == -1) {
            create(key, amount)
        } else {
     ...
     */
    fun sub(key: String, amount: Float) {
        add(key, -amount)
    }
     ...
     */
    fun mult(key: String?, amount: Float) {
        val index = index(key)
        if (index != -1) {
            values[index] *= amount
        }
     ...
     */
    fun div(key: String?, amount: Float) {
        val index = index(key)
        if (index != -1) {
            values[index] /= amount
        }
    ...
    protected fun create(what: String, much: Float) {
        if (count == keys.size) {
    ...
    protected fun sortImpl(useKeys: Boolean, reverse: Boolean,
        ...
        val s: Sort = object : Sort() {
            ...
            override fun compare(a: Int, b: Int): Int {
                var diff = 0f
                if (useKeys) {
                    diff = keys[a]!!.compareTo(keys[b]!!, ignoreCase = true).toFloat()
                    if (diff == 0f) {
