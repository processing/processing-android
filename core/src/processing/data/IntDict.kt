...
open class IntDict {
     ...
     * @nowebref
     */
    constructor(keys: Array<String?>, values: IntArray) {
    ...
    inner class Entry internal constructor(var key: String?, var value: Int)
    ...
    fun valueIterator(): MutableIterator<Int?> {
    ...
    operator fun get(key: String?, alternate: Int): Int {
        val index = index(key)
     ...
     */
    operator fun set(key: String, amount: Int) {
        val index = index(key)
        if (index == -1) {
            create(key, amount)
        } else {
    ...
    fun setIndex(index: Int, key: String, value: Int) {
     ...
     */
    fun mult(key: String?, amount: Int) {
        val index = index(key)
        if (index != -1) {
            values[index] *= amount
        }
    ...
    protected fun create(what: String, much: Int) {
        if (count == keys.size) {
