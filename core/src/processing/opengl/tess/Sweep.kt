...
import processing.opengl.tess.Dict.Companion.dictDelete
import processing.opengl.tess.Dict.Companion.dictDeleteDict
import processing.opengl.tess.Dict.Companion.dictInsert
import processing.opengl.tess.Dict.Companion.dictInsertBefore
import processing.opengl.tess.Dict.Companion.dictKey
import processing.opengl.tess.Dict.Companion.dictMin
import processing.opengl.tess.Dict.Companion.dictNewDict
import processing.opengl.tess.Dict.Companion.dictPred
import processing.opengl.tess.Dict.Companion.dictSearch
import processing.opengl.tess.Dict.Companion.dictSucc
import processing.opengl.tess.Geom.EdgeEval
import processing.opengl.tess.Geom.EdgeGoesLeft
import processing.opengl.tess.Geom.EdgeIntersect
import processing.opengl.tess.Geom.EdgeSign
import processing.opengl.tess.Geom.VertEq
import processing.opengl.tess.Geom.VertL1dist
...
import processing.opengl.tess.PriorityQ.Companion.pqNewPriorityQ
 ...
 */
    ...
    private fun RegionBelow(r: ActiveRegion?): ActiveRegion? {
        return dictKey(dictPred(r!!.nodeUp)) as ActiveRegion?
    }

    @JvmStatic
    private fun RegionAbove(r: ActiveRegion?): ActiveRegion? {
        return dictKey(dictSucc(r!!.nodeUp)) as ActiveRegion?
    }
        ...
        if (e1!!.Sym!!.Org == event) {
            return if (e2!!.Sym!!.Org == event) {
                ...
                if (VertLeq(e1.Org, e2.Org)) {
                    EdgeSign(e2.Sym!!.Org!!, e1.Org!!, e2.Org!!) <= 0
                } else EdgeSign(e1.Sym!!.Org!!, e2.Org!!, e1.Org!!) >= 0
            } else EdgeSign(e2.Sym!!.Org!!, event!!, e2.Org!!) <= 0
        }
        if (e2!!.Sym!!.Org == event) {
            return EdgeSign(e1.Sym!!.Org!!, event!!, e1.Org!!) >= 0
        }

        /* General case - compute signed distance *from* e1, e2 to event */t1 = EdgeEval(e1.Sym!!.Org!!, event!!, e1.Org!!)
        t2 = EdgeEval(e2.Sym!!.Org!!, event, e2.Org!!)
        return t1 >= t2
    }

    @JvmStatic
    fun DeleteRegion(tess: GLUtessellatorImpl, reg: ActiveRegion?) {
        if (reg!!.fixUpperEdge) {
 ...
 */
    {
        ...
        if (!__gl_meshDelete(reg.eUp!!)) return false
    ...
    @JvmStatic
    fun AddRegionBelow(tess: GLUtessellatorImpl,
                       regAbove: ActiveRegion?,
                       eNewUp: GLUhalfEdge?): ActiveRegion /*
 * Add a new active region to the sweep line, *somewhere* below "regAbove"
 * (according to where the new edge belongs in the sweep-line dictionary).
 * The upper edge of the new region will be "eNewUp".
 * Winding number and "inside" flag are not updated.
 */ {
        val regNew = ActiveRegion() ?: throw RuntimeException()
        regNew.eUp = eNewUp
        /* __gl_dictListInsertBefore */regNew.nodeUp = dictInsertBefore(tess.dict!!, regAbove!!.nodeUp, regNew)
    ...
    fun IsWindingInside(tess: GLUtessellatorImpl, n: Int): Boolean {
        when (tess.windingRule) {
            PGLU.GLU_TESS_WINDING_ODD -> return n and 1 != 0
            PGLU.GLU_TESS_WINDING_NONZERO -> return n != 0
            PGLU.GLU_TESS_WINDING_POSITIVE -> return n > 0
            PGLU.GLU_TESS_WINDING_NEGATIVE -> return n < 0
            PGLU.GLU_TESS_WINDING_ABS_GEQ_TWO -> return n >= 2 || n <= -2
        }
        throw InternalError()
        /*NOTREACHED*/
    }
    ...
    fun FinishRegion(tess: GLUtessellatorImpl, reg: ActiveRegion?) /*
 ...
 */
    ...
    @JvmStatic
    fun FinishLeftRegions(tess: GLUtessellatorImpl, regFirst: ActiveRegion?, regLast: ActiveRegion?): GLUhalfEdge?
 /*
 ...
 */
    {
        ...
        while (regPrev != regLast) {
            ...
            if (ePrev.Onext != e) {
                if (!__gl_meshSplice(e.Sym!!.Lnext!!, e)) throw RuntimeException()
                if (!__gl_meshSplice(ePrev, e)) throw RuntimeException()
            }
    ...
    @JvmStatic
    fun AddRightEdges(tess: GLUtessellatorImpl, regUp: ActiveRegion?,
 ...
 */
    {
        ...
        do {
            assert(VertLeq(e!!.Org, e.Sym!!.Org))
        ...
        while (true) {
            ...
            if (e.Onext != ePrev) {
                ...
                if (!__gl_meshSplice(e.Sym!!.Lnext!!, e)) throw RuntimeException()
                if (!__gl_meshSplice(ePrev.Sym!!.Lnext!!, e)) throw RuntimeException()
            }
            ...
            if (!firstTime && CheckForRightSplice(tess, regPrev)) {
                ...
                if (!__gl_meshDelete(ePrev)) throw RuntimeException()
            }
            firstTime = false
            regPrev = reg
            ePrev = e
    ...
    @JvmStatic
    fun CallCombine(tess: GLUtessellatorImpl, isect: GLUvertex?,
    ...
    @JvmStatic
    fun SpliceMergeVertices(tess: GLUtessellatorImpl, e1: GLUhalfEdge?,
                            e2: GLUhalfEdge?)
 /*
 ...
 */
    {
        ...
        if (!__gl_meshSplice(e1, e2)) throw RuntimeException()
    }

    @JvmStatic
    fun VertexWeights(isect: GLUvertex?, org: GLUvertex?, dst: GLUvertex?,
                      weights: FloatArray)
 /*
 ...
 */
    {
        val t1 = VertL1dist(org!!, isect!!)
        val t2 = VertL1dist(dst!!, isect)
    ...
    @JvmStatic
    fun GetIntersectData(tess: GLUtessellatorImpl, isect: GLUvertex?,
 ...
 */
    {
        ...
        if (VertLeq(eUp!!.Org, eLo!!.Org)) {
            ...
            /* eUp.Org appears to be below eLo */if (!VertEq(eUp.Org!!, eLo.Org!!)) {
                /* Splice eUp.Org into eLo */
                if (__gl_meshSplitEdge(eLo.Sym!!) == null) throw RuntimeException()
                if (!__gl_meshSplice(eUp, eLo.Sym!!.Lnext!!)) throw RuntimeException()
                regLo.dirty = true
            ...
            if (__gl_meshSplitEdge(eUp.Sym!!) == null) throw RuntimeException()
            if (!__gl_meshSplice(eLo.Sym!!.Lnext!!, eUp)) throw RuntimeException()
        }
        return true
    }

    @JvmStatic
    fun CheckForLeftSplice(tess: GLUtessellatorImpl?, regUp: ActiveRegion?): Boolean
 /*
 * Check the upper and lower edge of "regUp", to make sure that the
 ...
 */
    {
        ...
        val eLo = regLo!!.eUp
        val e: GLUhalfEdge?
        assert(!VertEq(eUp!!.Sym!!.Org!!, eLo!!.Sym!!.Org!!))
        if (VertLeq(eUp.Sym!!.Org, eLo.Sym!!.Org)) {
            ...
            if (e == null) throw RuntimeException()
            if (!__gl_meshSplice(eLo.Sym!!, e)) throw RuntimeException()
            ...
            if (e == null) throw RuntimeException()
            if (!__gl_meshSplice(eUp.Lnext!!, eLo.Sym!!)) throw RuntimeException()
 ...
 */
    {
         ...
         */
        orgMin = if (VertLeq(orgUp, orgLo)) orgUp else orgLo
        ...
        if ((!VertEq(dstUp, tess.event!!)
            ...
            if (dstLo == tess.event) {
                ...
                if (__gl_meshSplitEdge(eUp.Sym!!) == null) throw RuntimeException()
                if (!__gl_meshSplice(eLo.Sym!!, eUp)) throw RuntimeException()
            ...
            if (dstUp == tess.event) {
                ...
                if (__gl_meshSplitEdge(eLo.Sym!!) == null) throw RuntimeException()
                if (!__gl_meshSplice(eUp.Lnext!!, eLo.Sym!!.Lnext!!)) throw RuntimeException()
                regLo = regUp
 ...
 */
            ...
            if (eUp!!.Sym!!.Org != eLo!!.Sym!!.Org) {
                ...
                if (CheckForLeftSplice(tess, regUp)) {
                    ...
                    if (regLo.fixUpperEdge) {
                        ...
                        if (!__gl_meshDelete(eLo)) throw RuntimeException()
                        regLo = RegionBelow(regUp)
                        eLo = regLo!!.eUp
                    } else if (regUp.fixUpperEdge) {
                        DeleteRegion(tess, regUp)
                        if (!__gl_meshDelete(eUp)) throw RuntimeException()
    ...
    @JvmStatic
    fun ConnectRightVertex(tess: GLUtessellatorImpl, regUp: ActiveRegion?,
 ...
 */
    {
        ...
        if (VertEq(eUp.Org!!, tess.event!!)) {
            if (!__gl_meshSplice(eTopLeft!!.Sym!!.Lnext!!, eUp)) throw RuntimeException()
            regUp = TopLeftRegion(regUp)
            if (regUp == null) throw RuntimeException()
        ...
        if (VertEq(eLo.Org!!, tess.event!!)) {
            if (!__gl_meshSplice(eBottomLeft, eLo.Sym!!.Lnext!!)) throw RuntimeException()
         ...
         */
        eNew = if (VertLeq(eLo.Org, eUp.Org)) {
            eLo.Sym!!.Lnext!!
        } else {
            eUp
        }
 ...
 */
    ...
    @JvmStatic
    fun ConnectLeftDegenerate(tess: GLUtessellatorImpl,
 ...
 */
    {
        ...
        if (!VertEq(e.Sym!!.Org!!, vEvent)) {
            ...
            if (__gl_meshSplitEdge(e.Sym!!) == null) throw RuntimeException()
            if (regUp.fixUpperEdge) {
                /* This edge was fixable -- delete unused portion of original edge */
                if (!__gl_meshDelete(e.Onext!!)) throw RuntimeException()
                regUp.fixUpperEdge = false
            }
            if (!__gl_meshSplice(vEvent.anEdge!!, e)) throw RuntimeException()
        ...
        if (reg.fixUpperEdge) {
            ...
            DeleteRegion(tess, reg)
            if (!__gl_meshDelete(eTopRight)) throw RuntimeException()
            eTopRight = eTopLeft!!.Sym!!.Lnext
        }
        if (!__gl_meshSplice(vEvent.anEdge!!, eTopRight!!)) throw RuntimeException()
 ...
 */
    {
        ...
        regUp = dictKey(dictSearch(tess.dict!!, tmp)) as ActiveRegion?
        regLo = RegionBelow(regUp)
        eUp = regUp!!.eUp
        eLo = regLo!!.eUp
         ...
         */
        reg = if (VertLeq(eLo!!.Sym!!.Org, eUp.Sym!!.Org)) regUp else regLo
        ...
        if (regUp.inside || reg!!.fixUpperEdge) {
            if (reg == regUp) {
                ...
                val tempHalfEdge = __gl_meshConnect(eLo.Sym!!.Onext!!.Sym!!, vEvent.anEdge!!)
                        ?: throw RuntimeException()
 ...
 */
    private const val SENTINEL_COORD = 4.0 * PGLU.GLU_TESS_MAX_COORD
 ...
 */
    {
        val e: GLUhalfEdge?
        val reg = ActiveRegion() ?: throw RuntimeException()
 ...
 */
    {
        ...
        tess.dict = dictNewDict(tess, object : Dict.DictLeq {
            override fun leq(frame: Any?, key1: Any?, key2: Any?): Boolean {
                return EdgeLeq(tess, key1 as ActiveRegion?, key2 as ActiveRegion?)
            }
        })
    ...
    fun DoneEdgeDict(tess: GLUtessellatorImpl) {
        ...
        while ((dictKey(dictMin(tess.dict)) as ActiveRegion?).also { reg = it } != null) {
            /*
 ...
 */
    {
        ...
        while (e != eHead) {
            ...
            if (VertEq(e.Org!!, e.Sym!!.Org!!) && e.Lnext!!.Lnext != e) {
                ...
                if (!__gl_meshDelete(e)) throw RuntimeException() /* e is a self-loop */
                e = eLnext
                eLnext = e!!.Lnext
            }
            if (eLnext!!.Lnext == e) {
                ...
                if (eLnext != e) {
                    ...
                    if (!__gl_meshDelete(eLnext)) throw RuntimeException()
                }
                if (e == eNext || e == eNext!!.Sym) {
                    eNext = eNext.next
                }
                if (!__gl_meshDelete(e)) throw RuntimeException()
            }
            e = eNext
        }
    }
 ...
 */ {
        ...
        tess!!.pq = pqNewPriorityQ(object : PriorityQ.Leq {
            override fun leq(key1: Any?, key2: Any?): Boolean {
                return VertLeq(key1 as GLUvertex?, key2 as GLUvertex?)
            }
        })
 ...
 */
    {
        ...
        while ((tess.pq!!.pqExtractMin() as GLUvertex?).also { v = it } != null) {
            while (true) {
        ...
        /* __GL_DICTLISTKEY */ /* __GL_DICTLISTMIN */
        tess.event = (dictKey(dictMin(tess.dict)) as ActiveRegion?)!!.eUp!!.Org
        DebugEvent(tess)
        DoneEdgeDict(tess)
        DonePriorityQ(tess)
