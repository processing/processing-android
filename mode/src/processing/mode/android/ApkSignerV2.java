...
public class ApkSignerV2 {
    ...
    public static void signJarV2(File jarToSign, File outputJar, String alias, 
                                 ...
                                 NoSuchAlgorithmException, ApkParseException {
      ...
      SignerConfig conf = new SignerConfig();
      conf.privateKey = privateKey;
      conf.certificates = new ArrayList<>();
      conf.certificates.add(x509Cert);
      conf.signatureAlgorithms = new ArrayList<>();
      conf.signatureAlgorithms.add(SIGNATURE_RSA_PKCS1_V1_5_WITH_SHA256);
      List<SignerConfig> signerConfigs = new ArrayList<>();
      signerConfigs.add(conf);

      System.out.println("GETTING INPUT STREAM");
      FileInputStream inputStream = new FileInputStream(jarToSign);
      ByteBuffer inputBuffer = ByteBuffer.allocate(inputStream.available());
      while (inputStream.available() > 0) {
        inputBuffer.put((byte) inputStream.read());
      }
      System.out.println("V2 SIGNING...");
      inputBuffer.rewind();
