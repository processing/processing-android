...
internal class Permissions(var sketch: Sketch, var appComp: Int, var modeFolder: File) : JFrame("Android Permissions Selector") {
    ...
    var permissionScroller: JScrollPane
    var permissionList: JList<JCheckBox?> = CheckBoxList()
    var descriptionLabel: JLabel = JLabel()

    //   processing.core.PApplet.println("permissions are:");
   //    processing.core.PApplet.println(sel);

    private var selections: Array<String?>
        private get() {
            val sel = ArrayList<String?>()
            val model = permissionList.model as DefaultListModel<*>
            for (i in 0 until count) {
                if ((model[i] as JCheckBox).isSelected) {
                    sel.add(Companion.title[i])
                }
            }
...
//    processing.core.PApplet.println(sel);
            ...
            val model = permissionList.model as DefaultListModel<*>
            for (i in 0 until count) {
                val box = model[i] as JCheckBox
                //      System.out.println(map.containsKey(box.getText()) + " " + box.getText());
                box.isSelected = map.containsKey(box.text)
            }
        }

    private fun saveSelections() {
        val sel = selections
        val mf = Manifest(sketch, appComp, modeFolder, false)
        mf.permissions = sel
    }
    ...
    // constructor or initializer block for the Permissions class
    // will be executed before any other operation
    init {
        permissionList.addListSelectionListener { e ->
            if (!e.valueIsAdjusting) {
                val index = permissionList.selectedIndex
                if (index == -1) {
                    descriptionLabel.text = ""
                } else {
                    descriptionLabel.text = "<html>" + description[index] + "</html>"
                }
            }
        }

        permissionList.fixedCellHeight = CELL_HEIGHT
        permissionList.border = EmptyBorder(BORDER_VERT, BORDER_HORIZ,
        ...
        permissionList.addKeyListener(object : KeyAdapter() {
            override fun keyTyped(e: KeyEvent) {
                if (e.keyChar == ' ') {
                    val index = permissionList.selectedIndex
                    val checkbox = permissionList.model.getElementAt(index)!!
        ...
        vbox.border = EmptyBorder(BORDER, BORDER, BORDER, BORDER)
        outer.add(vbox)

        val labelText = getTextString("permissions.dialog.label")
        val urlText = getTextString("permissions.dialog.url", GUIDE_URL)
        val textarea = JLabel(labelText)
        val urlarea = JLabel(urlText)

        textarea.preferredSize = Dimension(TEXT_WIDTH, TEXT_HEIGHT)
        urlarea.preferredSize = Dimension(URL_WIDTH, URL_HEIGHT)
        urlarea.addMouseListener(object : MouseAdapter() {
            override fun mouseClicked(e: MouseEvent) {
                Platform.openURL(GUIDE_URL)
            }
        })

        urlarea.cursor = Cursor(Cursor.HAND_CURSOR)
        textarea.alignmentX = Component.LEFT_ALIGNMENT
        urlarea.alignmentX = Component.LEFT_ALIGNMENT

        vbox.add(textarea)
        vbox.add(urlarea)

        permissionScroller.alignmentX = Component.LEFT_ALIGNMENT
        vbox.add(permissionScroller)
        vbox.add(Box.createVerticalStrut(GAP))
//        descriptionLabel = JLabel()
        descriptionLabel.preferredSize = Dimension(DESC_WIDTH, DESC_HEIGHT)
        ...
        val dim = Dimension(Toolkit.getButtonWidth(),
    ...
    companion object {
        const val GUIDE_URL = "https://developer.android.com/training/articles/security-tips.html#Permissions"

        val BORDER_HORIZ = Toolkit.zoom(5)
        val BORDER_VERT = Toolkit.zoom(3)
        val CELL_HEIGHT = Toolkit.zoom(20)
        val BORDER = Toolkit.zoom(13)
        val TEXT_WIDTH = Toolkit.zoom(400)
        val TEXT_HEIGHT = Toolkit.zoom(80)
        val URL_WIDTH = Toolkit.zoom(400)
        val URL_HEIGHT = Toolkit.zoom(30)
        val DESC_WIDTH = Toolkit.zoom(400)
        val DESC_HEIGHT = Toolkit.zoom(50)
        val GAP = Toolkit.zoom(8)
        ...
        init {
            count = listing.size / 2
            title = arrayOfNulls(count)
            description = arrayOfNulls(count)
            for (i in 0 until count) {
                title[i] = listing[i * 2]
...
public class CheckBoxList : JList<JCheckBox?>() {
    var checkboxWidth: Int

    inner class CellRenderer : ListCellRenderer<JCheckBox> {
        override fun getListCellRendererComponent(list: JList<out JCheckBox>,
                                                  checkbox: JCheckBox,
                                                  index: Int, isSelected: Boolean,
                                                  cellHasFocus: Boolean): Component {
            checkbox.background = if (isSelected) selectionBackground else background
            checkbox.foreground = if (isSelected) selectionForeground else foreground
            checkbox.isEnabled = list.isEnabled
            checkbox.font = font
            checkbox.isFocusPainted = false
            checkbox.isBorderPainted = true
            checkbox.border = if (isSelected) UIManager.getBorder("List.focusCellHighlightBorder") else noFocusBorder
            return checkbox
        }
    }
    ...
    companion object {
        @JvmStatic
        var noFocusBorder: Border = EmptyBorder(1, 1, 1, 1)
    }

    // constructor or initializer block of the CheckBoxList class
    // will be executed before any other operation for this class
    init {
        cellRenderer = CellRenderer() as ListCellRenderer<in JCheckBox?>

        // get the width of a checkbox so we can figure out if the mouse is inside
        checkboxWidth = JCheckBox().preferredSize.width
