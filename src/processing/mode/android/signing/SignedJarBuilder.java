...
public class SignedJarBuilder {
  ...
  /**
   * Check for extension implementations and if found use the contributed class
   * to write the signature block.  Otherwise use the default implementation.
   *
   * @param signature
   * @param publicKey
   * @param privateKey
   ...
   * @throws GeneralSecurityException
   */
  private void writeSignature(Signature signature, X509Certificate publicKey,
                              PrivateKey privateKey)
      throws IOException, GeneralSecurityException{
      writeSignatureBlock(signature, publicKey, privateKey);
  }
}