...
//  static private final String HANDHELD_LAYOUT_TEMPLATE = "HandheldLayout.xml.tmpl";
   ...
   * @param appComp component (reglar app, wallpaper, watchface, VR)
  ...
  public File build(String target) throws IOException, SketchException {
    ...
    if (appComponent == WATCHFACE && !runOnEmulator) {
      // We are building a watchface to run on the device, not in the emulator. 
      // We need the handheld app:
      ...
      File wearFolder = createProject(true, true);
   ...
   * wallpapers, and top, mobile and wear modules in the case of watchfaces.
   */
...
/*        createTopModule(projectFolder, exportFolder, "':mobile', ':wear'");
   ...
   * from the watch.
   */  
  protected File createHandheldProject(File wearFolder, File wearPackage) 
      throws IOException, SketchException {
  ...
  private void createTopModule(String buildToolsVer, String projectModules) throws IOException {
...
//    }
    ...
    if (getAppComponent() == FRAGMENT) {
      File aarFile = new File(sdk.getSupportLibrary(), 
          "/appcompat-v7/" + support_version + "/appcompat-v7-" + support_version + ".aar");
...
//    drawableFolder.mkdirs()
  ...
  private void copySupportLibs(File tmpFolder, File libsFolder) throws IOException {
    ...
    // vector-drawable)
    File aarFile = new File(sdk.getSupportLibrary(), 
        "/support-core-utils/" + support_version + "/support-core-utils-" + support_version + ".aar");
    Util.copyFile(aarFile, new File(libsFolder, aarFile.getName()));
    
    aarFile = new File(sdk.getSupportLibrary(), 
        "/support-compat/" + support_version + "/support-compat-" + support_version + ".aar");
    Util.copyFile(aarFile, new File(libsFolder, aarFile.getName()));
    
    aarFile = new File(sdk.getSupportLibrary(), 
        "/support-fragment/" + support_version + "/support-fragment-" + support_version + ".aar");
    Util.copyFile(aarFile, new File(libsFolder, aarFile.getName()));

    aarFile = new File(sdk.getSupportLibrary(), 
        "/support-vector-drawable/" + support_version + "/support-vector-drawable-" + support_version + ".aar");
    Util.copyFile(aarFile, new File(libsFolder, aarFile.getName()));
    
    File compatJarFile = new File(sdk.getSupportLibrary(), 
        "/support-annotations/" + support_version + "/support-annotations-" + support_version + ".jar");
...
//        System.out.println("Library project referenced succesfully!");
  ...
  public File exportProject() throws IOException, SketchException {
    ...
    if (appComponent == WATCHFACE) {
      ...
      // handheld app:
      File wearFolder = createProject(false, true);
      exportProject = false;
      if (wearFolder == null) return null;
      if (!antBuild()) return null;
  ...
  private File signPackage(File projectFolder, String keyStorePassword) throws Exception {
    ...
    File unsignedPackage = new File(projectFolder, 
        "app/build/outputs/apk/" + sketch.getName().toLowerCase() + "_release_unsigned.apk");
    if (!unsignedPackage.exists()) return null;
    File signedPackage = new File(projectFolder, 
        "app/build/outputs/apk/" + sketch.getName().toLowerCase() + "_release_signed.apk");
  ...
  private File zipalignPackage(File signedPackage, File projectFolder) throws IOException, InterruptedException {
    ...
    File alignedPackage = new File(projectFolder, 
        "app/build/outputs/apk/" + sketch.getName().toLowerCase() + "_release_signed_aligned.apk");
  ...
  private boolean antBuild() throws SketchException {
  ...
  private void antBuildProblems(String outPile, String errPile) throws SketchException {
...
//            return false; // get outta here
  ...
  private boolean referenceLibraryProject(String target, String path, String lib) {
    final String[] params = {
        sdk.getAndroidToolPath(),
        "update", "project",
        "--target", target,
